...
HRESULT KinectFusionProcessor::InitializeKinectFusion()
{
	...
	if (nullptr == m_pCameraPoseFinder)
	{
		...
		cameraPoseFinderParameters.featureSampleLocationsPerFrameCount = m_paramsCurrent.m_glCameraPoseFinderFeatureSampleLocationsPerFrame;
...
void KinectFusionProcessor::ProcessDepth()
{
	...
	if (m_paramsCurrent.m_bAutoFindCameraPoseWhenLost && !m_bTrackingHasFailedPreviously
		...
		&& m_cFrameCounter % m_paramsCurrent.m_glCameraPoseFinderProcessFrameCalculationInterval == 0
		&& colorSynchronized)
	{
...
HRESULT KinectFusionProcessor::CalculateMesh(INuiFusionColorMesh** ppMesh, int voxelStep)
{
	AssertOtherThread();
	...
	{
		hr = m_pVolume->CalculateMesh(voxelStep, ppMesh);
