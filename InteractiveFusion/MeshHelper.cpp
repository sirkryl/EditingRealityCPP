...
namespace InteractiveFusion {
	...
	std::vector<shared_ptr<MeshContainer>> meshData;
	shared_ptr<MeshContainer> originalMesh(new MeshContainer);

	void MeshHelper::InitialLoadFromFile(const char* fileName)
	{
		if (originalMesh->GetNumberOfVertices() != 0)
			originalMesh->CleanUp();
	...
	void MeshHelper::FillHoles(int holeSize)
	{
		...
		{
			cnt++;
			DebugUtility::DbgOut(L"fill hole #", cnt);
			(*mI)->CopyVisibleToInternalData()
			holeCnt += (*mI)->FillHoles(holeSize);
			(*mI)->CopyInternalToVisibleData();
			(*mI)->InitializeEssentials();
			(*mI)->UpdateBuffers();

			appStatus.SetStatusBarMessage(L"Filling holes..." + to_wstring(cnt + 1) + L"% of " + to_wstring(meshData.size()));
		}
		appStatus.SetStatusBarMessage(L"Filled " + to_wstring(holeCnt) + L" holes in " + to_wstring(cnt) + L"segments.");
	}
	...
	void MeshHelper::FillHoles(int index, int holeSize)
	{
		...
		int holeCnt = 0;
		DebugUtility::DbgOut(L"fill hole #", cnt);
		meshData[index]->CopyVisibleToInternalData()
		holeCnt += meshData[index]->FillHoles(holeSize);
		meshData[index]->CopyInternalToVisibleData();
	...
	void MeshHelper::RemoveSmallComponents(int size)
	{
		...
		{
			cnt++;
			DebugUtility::DbgOut(L"remove components #", cnt);
			cmpCnt += (*mI)->RemoveSmallComponents(size);
			(*mI)->CopyInternalToVisibleData();
			(*mI)->UpdateBuffers();
		}
		appStatus.SetStatusBarMessage(L"Deleted " + to_wstring(cmpCnt) + L"components");
	}
	...
	void MeshHelper::ResetAll()
	{
		...
		{
			meshData[removeIndices[i] - cnt]->CleanUp();
	...
	int MeshHelper::DuplicateMesh(int index)
	{
		...
		mesh->CopyToInternalData(meshData[index]->GetVertices(), meshData[index]->GetTriangles());
	...
	void MeshHelper::CombineAndExport()
	{
		...
		for (vector <shared_ptr<MeshContainer>>::iterator mI = meshData.begin(); mI != meshData.end(); ++mI)
		{
			if (!(*mI)->IsDeleted())
			{
				std::vector<Triangle> indices = (*mI)->GetTriangles();
				int faceCount = 0;
		...
		//DebugUtility::DbgOut(L"the path is : %s\n", ofn.lpstrFile);
		//getchar();
		//TCHAR iFileName;
	...
	void MeshHelper::Export(int index, string _fileName, bool saveDialog, bool align)
	{
		...
		std::vector<Triangle> indices = meshData[index]->GetTriangles();
		for (int i = 0; i < indices.size(); i += 1){
	...
	void MeshHelper::GenerateOriginalBuffers()
	{
		...
		{
			DebugUtility::DbgOut(L"INITIALIZE ONCE");
	...
	{
		DebugUtility::DbgOut(L"MeshHelper Generate Buffers");
		for (vector <shared_ptr<MeshContainer>>::iterator mI = meshData.begin(); mI != meshData.end(); ++mI)
		{
			(*mI)->GenerateBOs();
	...
	{
		originalMesh->HighlightTrianglesWithColor(triangles, color, additive);
	}

	void MeshHelper::HighlightObjects(int index, std::vector<int> triangles, ColorIF color, bool additive)
	{
		meshData[index]->HighlightTrianglesWithColor(triangles, color, additive);
	}
	...
	{
		//DebugUtility::DbgOut(L"Meshhelper DrawAll");
		for (vector <shared_ptr<MeshContainer>>::iterator mI = meshData.begin(); mI != meshData.end(); ++mI)
		{
			(*mI)->Draw();
		}
	...
	{
		//DebugUtility::DbgOut(L"Meshhelper DrawAllForColorPicking");
		for (vector <shared_ptr<MeshContainer>>::iterator mI = meshData.begin(); mI != meshData.end(); ++mI)
		{
			if (!(*mI)->IsPlane())
				(*mI)->DrawForColorPicking();
		}
		//shaderColor.UnUseProgram();
	}
	...
	glm::vec3 MeshHelper::GetCombinedCenterPoint()
	{
		...
		DebugUtility::DbgOut(L"centerPoint x: ", centerPoint.x);
		DebugUtility::DbgOut(L"centerPoint y: ", centerPoint.y);
		DebugUtility::DbgOut(L"centerPoint z: ", centerPoint.z);
		return centerPoint;
	}
	...
	void MeshHelper::CleanAndParse(const char* fileName, std::vector<Vertex> &startingVertices, std::vector<Triangle> &startingIndices)
	{
		...
		if (mesh.vn > 1000)
		{
			...
			DebugUtility::DbgOut(_T("Removed duplicates:"), dup);
			int unref = vcg::tri::Clean<VCGMesh>::RemoveUnreferencedVertex(mesh);
			DebugUtility::DbgOut(_T("Removed unreferenced:"), unref);
			int deg = vcg::tri::Clean<VCGMesh>::RemoveDegenerateFace(mesh);
			DebugUtility::DbgOut(_T("Removed degenerate faces:"), deg);
		}
		...
		DebugUtility::DbgOut(L"Parse duration: ", duration);
